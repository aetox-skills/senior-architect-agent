...
### 6.2 สิ่งที่อยู่นอกขอบเขตของ Core Skill

สิ่งเหล่านี้ไม่ควรถูกยัดเข้ามาเป็นแกนหลักของ core repository ในช่วงแรก:
...
- large multi-example suite
- full schema validation system
- complex generator engine
- integration กับหลาย platform
- advanced visualization engine
- SVG exporter system ในฐานะเครื่องมือซับซ้อนหรือระบบอัตโนมัติเต็มรูปแบบ

ข้อสำคัญ:

> การมีไฟล์ SVG เป็น output ไม่ได้ขัดกับขอบเขตของโปรเจค  
> สิ่งที่อยู่นอกขอบเขตคือการทำระบบ exporter ขนาดใหญ่จนโปรเจคกลายเป็น tooling platform

เหตุผลไม่ใช่เพราะสิ่งเหล่านี้ไม่ดี  
แต่เพราะ core skill ต้องรักษาความคมของการเป็น architecture skill ก่อน ไม่ใช่กลายเป็น platform
ที่ขอบเขตเบลอ
...
v0.1.0 ยังไม่ควรเน้น CLI, plugin, automation หรือ exporter system ขนาดใหญ่
...
โดยยังไม่ขยายเป็น tooling platform
...
- SVG visual artifact role ที่อธิบายไว้ชัด แต่ยังไม่ต้องสร้างไฟล์ `.svg`
  ใน v1
- `agents/openai.yaml` เป็น interface metadata เท่านั้น

v1.0.0 ยังไม่ควรทำ CLI, web UI, plugin platform, automation framework
หรือ SVG exporter system
...
- SVG export/artifact guidelines
- README-embeddable architecture maps
- diagram examples สำหรับระบบซับซ้อน

เวอร์ชันนี้ยังไม่จำเป็นต้องเป็น exporter system เต็มรูปแบบ  
เป้าหมายคือทำให้ visual output มีมาตรฐานและไม่มั่ว

---

### v1.0.0 - Stable Architecture Discipline Standard

เป้าหมายคือเป็นมาตรฐานที่สามารถนำไปใช้กับโปรเจคจริงได้อย่างมั่นใจ

ควรมี:

- operating modes ที่ชัด
- rules ที่ไม่บวม
- templates ที่ใช้ได้จริง
- examples ที่พิสูจน์พฤติกรรม
- documentation ที่อ่านง่าย
- output ที่มนุษย์และ AI ใช้ต่อได้
- Mermaid diagram standards
- SVG visual artifact guidelines
- release discipline
- changelog
- clear scope
- practical README
...
### 13.6 อย่าให้โปรเจคขยายเร็วกว่าแกน

ฟีเจอร์เสริม เช่น CLI, plugin, web UI และ automation framework ควรมาหลังจากแกนทำงานชัดแล้ว
หรือแยกเป็นโปรเจค/extension ภายนอก

SVG visual artifact อยู่ในขอบเขตได้เมื่อช่วยให้เห็นภาพชัด  
แต่ SVG exporter system ขนาดใหญ่ไม่ควรถูกยัดเข้ามาเป็นแกนของ core skill ในช่วงแรก
...
## 15. ขอบเขตที่ไม่ควรปนกับ Core Skill

โปรเจคนี้ไม่ควรถูกขยายให้กลายเป็น CLI, plugin platform, automation framework หรือ tooling platform
ภายใน core repository

สิ่งเหล่านั้นอาจเป็นส่วนต่อยอดในอนาคต แต่ไม่ใช่แกนหลักของ `senior-architect-agent`
...
เหตุผลคือโปรเจคนี้ต้องรักษาความคมของการเป็น architecture skill ไม่ใช่กลายเป็น tooling platform
ที่ขอบเขตเบลอ
